...
private:
	...
	void addToSearchMap(const std::vector<unsigned char>& bytes, const std::vector<unsigned char>& mask, uintptr_t start, uintptr_t end);
	bool findInSearchMap(const SearchMapKey& key, SearchMapValue& region, bool allowAdd, SearchMapValue& originalRegion);
	void getOrAddToSearchMap8Byte(const unsigned char* bytes, const unsigned char* mask, int size, SearchMapValue& region, bool allowAdd, SearchMapValue& originalRegion);
	void getOrAddToSearchMap(const std::vector<unsigned char>& bytes, const std::vector<unsigned char>& mask, SearchMapValue& region, bool allowAdd);
...
public:
	...
	void* findSignatureFast1(const std::vector<unsigned char>& bytes, const std::vector<unsigned char>& mask, uintptr_t start, uintptr_t end);
	template <bool forward>
	void* findSignatureFast8(const std::vector<unsigned char>& bytes, const std::vector<unsigned char>& mask, uintptr_t start, uintptr_t end);
	template <bool forward>
	void* findSignatureFast32(const std::vector<unsigned char>& bytes, const std::vector<unsigned char>& mask, uintptr_t start, uintptr_t end);

	template <bool forward> // start inclusive, end exclusive
